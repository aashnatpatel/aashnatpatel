  ...
  return (
    <div className="min-h-screen bg-cream font-body">
      ...
      <nav className="fixed top-0 w-full z-40 bg-cream border-b border-warm-gray px-8 md:px-16 py-5 flex justify-between items-center">
        <div className="hidden md:flex gap-8 text-sm tracking-wide text-forest">
        ...
        >
          <div className="space-y-1.5">
            <span className={`block w-6 h-px bg-forest-dark transition-all ${menuOpen ? 'rotate-45 translate-y-2' : ''}`}></span>
            <span className={`block w-6 h-px bg-forest-dark transition-all ${menuOpen ? 'opacity-0' : ''}`}></span>
            <span className={`block w-6 h-px bg-forest-dark transition-all ${menuOpen ? '-rotate-45 -translate-y-2' : ''}`}></span>
          </div>
        </button>
      ...
      {menuOpen && (
        <div className="fixed top-16 left-0 w-full bg-cream border-b border-warm-gray z-30 flex flex-col px-8 py-6 gap-6 md:hidden">
          {['Work', 'Projects', 'About', 'Contact'].map((item) => (
            <a
              key={item}
              href={`#${item.toLowerCase()}`}
              className="text-sm tracking-wide text-forest hover:text-rose transition-colors"
      ...
      <section className="min-h-screen relative flex flex-col items-center justify-center text-center px-8 md:px-16 pt-24 pb-16 overflow-hidden">
        ...
        <div className="relative z-10">
          <p className="text-rose text-xs font-medium tracking-widest uppercase mb-6">Available for work</p>
          <h1 className="font-heading text-5xl md:text-8xl text-forest-dark leading-tight mb-6">
            Aashna Patel
          </h1>
          <p className="font-heading text-xl md:text-3xl text-forest italic leading-snug mb-6 max-w-2xl">
            Growth marketer with a builder's mindset.
          </p>
          <p className="text-forest text-xs tracking-widest uppercase mb-10">
            GTM · Growth · Marketing · Product
          </p>
          <div className="flex gap-4 justify-center">
            <a href="#work" className="bg-forest text-cream px-6 py-3 text-xs font-medium tracking-widest uppercase hover:bg-forest-dark transition-colors">
              See My Work
            </a>
            <a href="#contact" className="border border-forest text-forest px-6 py-3 text-xs font-medium tracking-widest uppercase hover:bg-warm-gray transition-colors">
              Contact
            </a>
          </div>
        </div>
      ...
      {/* Work */}
      <section id="work" className="px-8 md:px-16 py-16 md:py-32 border-t border-warm-gray">
        <FadeIn>
          <p className="text-rose text-xs font-medium tracking-widest uppercase mb-2">Experience</p>
          <h2 className="font-heading text-4xl md:text-5xl text-forest-dark mb-8 md:mb-16">Work</h2>
        </FadeIn>
        <div className="space-y-0">
          {[
            ...
            <FadeIn key={i} delay={i * 0.1}>
              <div className="border-t border-warm-gray py-8 md:py-12 grid md:grid-cols-3 gap-4 md:gap-8">
                <div>
                  <a href={job.url} target="_blank" rel="noopener noreferrer" className="block mb-2 hover:opacity-60 transition-opacity">
                    <img src={job.logo} alt={job.company} className={`${job.logoHeight} object-contain object-left`} />
                  </a>
                  <p className="text-sm text-forest">{job.period} · {job.location}</p>
                </div>
                <div className="md:col-span-2">
                  <p className="text-xs font-medium tracking-wide text-forest-dark uppercase mb-3">{job.title}</p>
      ...
      {/* Projects */}
      <section id="projects" className="px-8 md:px-16 py-16 md:py-32 border-t border-warm-gray">
        <FadeIn>
          <p className="text-rose text-xs font-medium tracking-widest uppercase mb-2">Built</p>
          <h2 className="font-heading text-4xl md:text-5xl text-forest-dark mb-8 md:mb-16">Projects</h2>
        </FadeIn>
        <div className="space-y-0">
          <FadeIn>
            <div className="border-t border-warm-gray py-8 md:py-12">
              <div className="flex flex-col md:flex-row gap-8 md:gap-16 md:items-center">
                <div className="flex-1">
                  ...
                  <p className="font-heading text-2xl md:text-3xl text-forest-dark mb-2">SBHA Care Manager</p>
                  <p className="text-sm text-forest italic mb-4">A full-stack patient management web app built for a real healthcare advocacy business.</p>

                  {/* Mobile screenshot */}
                  <div className="md:hidden mb-4">
                    <div className="flex justify-end mb-1">
                      <a href="https://sbha-care-manager.vercel.app" target="_blank" rel="noopener noreferrer" className="text-rose text-xs tracking-wide hover:underline">
                        View live site →
                      </a>
                    </div>
                    <img
                      ...
                      className="w-full border border-warm-gray cursor-pointer"
                      onClick={() => setLightbox(sbhaScreenshot)}
                    />
                    <p className="text-xs text-forest text-center mt-1">Tap to enlarge</p>
                  </div>
      ...
      {/* About */}
      <section id="about" className="px-8 md:px-16 py-16 md:py-32 border-t border-warm-gray">
        <FadeIn>
          <p className="text-rose text-xs font-medium tracking-widest uppercase mb-2">About</p>
          <h2 className="font-heading text-4xl md:text-5xl text-forest-dark mb-8 md:mb-16">I'm Aashna</h2>
        </FadeIn>
        <FadeIn delay={0.1}>
          <div className="flex flex-col md:flex-row gap-8 md:gap-12 md:items-center">
            <div className="md:hidden w-40 mx-auto">
              <div className="relative">
                <div className="absolute -bottom-3 -right-3 w-full h-full border-2 border-rose"></div>
                <img src={headshot} alt="Aashna Patel" className="relative w-full object-cover object-top" />
              </div>
            </div>
            ...
            <div className="flex-1">
              <p className="text-forest text-sm md:text-base leading-relaxed mb-8">
              ...
              <p className="text-rose text-xs font-medium tracking-widest uppercase mb-6">Beyond my resume</p>
              <div className="space-y-5">
                {[
                  ...
                  <div key={i} className="flex gap-4 items-start">
                    ...
                    <p className="text-forest text-sm md:text-base leading-relaxed">{fact}</p>
                  </div>
                ))}
              </div>
            </div>
      ...
      {/* Contact */}
      <section id="contact" className="px-8 md:px-16 py-16 md:py-32 border-t border-warm-gray">
        ...
        <FadeIn>
          <div className="flex flex-col items-center text-center mt-4 md:mt-8">
            <h2 className="font-heading text-4xl md:text-5xl text-forest-dark mb-4 md:mb-6">Let's talk</h2>
            <p className="text-forest text-sm md:text-base mb-8 max-w-md">
              Open to new opportunities, collaborations, or just a good conversation — my inbox is always open.
            </p>
            <a href="mailto:aashnatpatel@gmail.com" className="font-heading text-2xl md:text-4xl text-rose hover:opacity-75 transition-opacity block mb-8">
              aashnatpatel@gmail.com
            </a>
            <div className="flex gap-8 mb-8">
              <a href="https://www.linkedin.com/in/aashnatpatel/" target="_blank" rel="noopener noreferrer" className="text-sm text-forest-dark border-b border-forest-dark hover:text-rose hover:border-rose transition-colors pb-1">
                LinkedIn
              </a>
              <a href="https://github.com/aashnatpatel" target="_blank" rel="noopener noreferrer" className="text-sm text-forest-dark border-b border-forest-dark hover:text-rose hover:border-rose transition-colors pb-1">
                GitHub
              </a>
            </div>
            <a href={resume} download="Aashna_Patel_Resume.pdf" className="text-xs text-rose border border-rose px-6 py-3 tracking-widest uppercase hover:bg-rose hover:text-cream transition-colors">
              Download Resume
            </a>
          </div>
        </FadeIn>
